...
class AgentControlNode(Node):
    def __init__(self):
        ...
        self.next_point = self.map_to_webots_transform(self.path.pop(0))
        self.next_point.position.z = 1.0
    ...
    def control_cycle(self):
        print(f"Current Point: {self.next_point}")
        if len(self.path) == 0:
            self.next_point.position.z = 0.0
        elif (
            (self._state.position.x - self.next_point.position.x) ** 2
            + (self._state.position.y - self.next_point.position.y) ** 2
        ) ** 0.5 < self.pos_threshold:
            self.next_point = self.map_to_webots_transform(self.path.pop(0))
            self.next_point.position.z = 1.0

        # publish the setpoint
        self._pos_setpoint_publisher.publish(self.next_point)
        # self._pos_setpoint_publisher.publish(self.next_point)
